...
useEffect(() => {
  fetch('/api/limiteds')
    .then(res => res.json())
    .then(data => setLimiteds(data.limitedsList || []))
    .catch(err => console.error('Failed to fetch limiteds', err));
}, []);
...
const renderNavTabs = () => (
  <div className="flex justify-center space-x-4 mb-4">
    <button
      className={viewType === 'robux' ? 'bg-blue-600 text-white px-4 py-2 rounded' : 'bg-gray-200 px-4 py-2 rounded'}
      onClick={() => setViewType('robux')}
    >
      ROBUX LIST
    </button>
    <button
      className={viewType === 'limiteds' ? 'bg-blue-600 text-white px-4 py-2 rounded' : 'bg-gray-200 px-4 py-2 rounded'}
      onClick={() => setViewType('limiteds')}
    >
      LIMITED LIST
    </button>
  </div>
...
// Card component for limiteds (glass style)
const LimitedCard = ({ item }) => (
  <motion.div
    layout
    className="bg-white/10 backdrop-blur-md p-4 rounded-lg shadow-md text-white"
  >
    <img src={item.imageUrl} alt="limited thumb" className="w-32 h-32 object-cover rounded mb-2" />
    <p className="font-bold">{item.itemName}</p>
    <p className="text-sm">By: {item.creator}</p>
    <p className="text-sm">Price: {item.price} R$</p>
    <a href={item.contactLink} className="text-blue-300 underline text-sm" target="_blank" rel="noreferrer">Contact</a>
    <p className="text-sm mt-1">Seller: {item.sellerName}</p>
  </motion.div>
);

// Add rendering logic for limiteds
const renderLimiteds = () => (
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6">
    <AnimatePresence>
      {limiteds.map(item => (
        <LimitedCard key={item.id} item={item} />
      ))}
    </AnimatePresence>
  </div>
);

// Replace main render block
return (
  <div className="p-4">
    {renderNavTabs()}
    {viewType === 'robux' && (
      <div>
        {/* existing robux listings here */}
      </div>
    )}
    {viewType === 'limiteds' && renderLimiteds()}
  </div>
);
