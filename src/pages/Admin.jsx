...
export default function Admin() {
  ...
  assetId: '', price: '', contact: '', poisonStatus: 'Safe'
});
const [limitedList, setLimitedList] = useState([]);
const handleLimitedSubmit = async (e) => {
  e.preventDefault();
  try {
    const response = await axios.post('/api/limited', limitedForm);
    setLimitedList([...limitedList, response.data]);
    setLimitedForm({ assetId: '', price: '', contact: '', poisonStatus: 'Safe' });
  } catch (err) {
    console.error(err);
  }
};
const handleLimitedEdit = (item) => {
  // Logic to populate form with item data for editing
  setLimitedForm(item);
};
const handleLimitedDelete = async (id) => {
  try {
    await axios.delete(`/api/limited/${id}`);
    setLimitedList(limitedList.filter(item => item._id !== id));
  } catch (err) {
    console.error(err);
  }
};
