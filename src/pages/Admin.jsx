...
export default function Admin() {
  ...
  useEffect(() => {
    fetch('/api/limiteds')
      ...
      .catch(err => console.error('Failed to fetch limited listings:', err));
  }, []);

  const handleLimitedSubmit = async () => {
    if (!limitedForm.assetId || !limitedForm.price || !limitedForm.contactLink || !seller?.username) {
      return Swal.fire('Error', 'Fill all required fields.', 'error');
    }

    const method = editLimitedId ? 'PUT' : 'POST';
    const payload = {
      ...limitedForm,
    ...
    if (res.ok) {
      Swal.fire('Success', editLimitedId ? 'Listing updated' : 'Listing added', 'success');
      setLimitedForm({
        assetId: '',
        itemName: '',
        creator: '',
  ...
  const handleLimitedEdit = (item) => {
    setLimitedForm(item);
    setEditLimitedId(item.id);
  };

  const handleLimitedDelete = async (id) => {
    await fetch('/api/limiteds', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id })
    });
    Swal.fire('Deleted', 'Limited listing removed.', 'success');
    fetch('/api/limiteds')
      .then(res => res.json())
      .then(data => setLimiteds(data.limitedsList || []));
  };
