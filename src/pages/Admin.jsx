...
export default function Admin() {
  ...
  const [formView, setFormView] = useState("account");
  const [limited, setLimited] = useState({ assetId: "", price: "" });
  const handleLimitedSubmit = async () => {
    if (!limited.assetId || !limited.price) {
    ...
    try {
      const response = await fetch("/api/limited", {
        ...
        body: JSON.stringify({ ...limited, seller: seller?.username || "admin" }),
      });
      const data = await response.json();
      if (data.success) {
        Swal.fire("Success", "Limited item listed successfully", "success");
        setLimited({ assetId: "", price: "" });
      } else {
        Swal.fire("Error", data.message || "Failed to list item", "error");
      }
    } catch (error) {
  ...
  if (!isAuthorized && !seller) {
    return (
      <div className="container" style={{ padding: '20px' }}>
        ...
        <div style={{ marginTop: '20px', display: 'flex', gap: '10px' }}>
          <button onClick={() => setFormView("account")} style={{ padding: "10px", backgroundColor: formView === "account" ? "#7DC387" : "#ccc" }}>Account Listing Form</button>
          <button onClick={() => setFormView("robux")} style={{ padding: "10px", backgroundColor: formView === "robux" ? "#7DC387" : "#ccc" }}>Robux Listing Form</button>
    ...
    <div className="container" style={{ padding: '20px' }}>
      ...
      {formType === 'robux' && (
        ...
        {formView === "limited" && (
          <div style={{ marginTop: "20px", padding: "20px", border: "1px solid #555", borderRadius: "10px" }}>
            <h3 style={{ color: "white" }}>List Limited Item</h3>
            <input type="text" placeholder="Asset ID" value={limited.assetId} onChange={(e) => setLimited({ ...limited, assetId: e.target.value })} required />
            <input type="number" placeholder="Price" value={limited.price} onChange={(e) => setLimited({ ...limited, price: e.target.value })} required />
            <button onClick={handleLimitedSubmit} style={{ marginTop: "10px" }}>List Limited Item</button>
          </div>
        )}
