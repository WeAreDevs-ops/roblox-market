...
// --- LIMITED LIST SECTION START ---
import axios from 'axios';

const LimitedItemCard = ({ item }) => (
  <div style={{
    border: '1px solid #ccc',
    borderRadius: '12px',
    padding: '16px',
    ...
    <img src={item.image} alt={item.itemName} style={{ width: '150px', height: '150px' }} />
    <h3>{item.itemName}</h3>
    <p><strong>Creator:</strong> {item.creator}</p>
    <p><strong>Is Limited:</strong> {item.isLimited ? 'Yes' : 'No'}</p>
    <p><strong>Is Limited Unique:</strong> {item.isLimitedUnique ? 'Yes' : 'No'}</p>
    <p><strong>Robux Price:</strong> {item.robuxPrice ?? 'N/A'}</p>
    <p><strong>Price (PHP):</strong> {item.pricePHP}</p>
    <p><strong>Contact:</strong> <a href={item.contactLink} target="_blank" rel="noreferrer">{item.contactLink}</a></p>
    <p><strong>Status:</strong> {item.poisonStatus}</p>
  </div>
);
...
const LimitedList = () => {
  ...
  const [poisonStatus, setPoisonStatus] = useState('Safe');
  ...
  const fetchLimited = async () => {
    try {
      ...
      const resellersRes = await axios.get(`https://economy.roblox.com/v1/assets/${assetId}/resellers?limit=1`);
      const robuxPrice = resellersRes?.data?.data?.[0]?.price;
  ...
  return (
    <div>
      ...
      <input type="text" placeholder="Price (PHP)" value={pricePHP} onChange={(e) => setPricePHP(e.target.value)} />
      <input type="text" placeholder="Contact Link" value={contactLink} onChange={(e) => setContactLink(e.target.value)} />
      <select value={poisonStatus} onChange={(e) => setPoisonStatus(e.target.value)}>
        <option value="Safe">Safe</option>
        <option value="Poison">Poison</option>
      </select>
      <button onClick={fetchLimited}>Add Limited</button>

      {limitedData && <LimitedItemCard item={limitedData} />}
    </div>
  );
};


// --- LIMITED LIST SECTION END ---
