...
const [limitedItemData, setLimitedItemData] = useState(null);
const [thumbUrl, setThumbUrl] = useState('');
const [editLimitedId, setEditLimitedId] = useState(null);

const fetchLimitedInfo = async () => {
  try {
    const res = await axios.get(
      `https://economy.roblox.com/v2/assets/${limitedForm.assetId}/details`
    );
    const item = res.data;
    setLimitedItemData(item);

    const thumbRes = await axios.get(
      `https://thumbnails.roblox.com/v1/assets?assetIds=${limitedForm.assetId}&size=420x420&format=Png`
    );
    setThumbUrl(thumbRes.data.data[0]?.imageUrl || '');
  } catch (err) {
    Swal.fire('Error', 'Failed to fetch item details. Check Asset ID.', 'error');
  }
};

const handleLimitedSubmit = () => {
  const newItem = {
    ...limitedForm,
    itemName: limitedItemData?.Name,
    creator: limitedItemData?.Creator?.Name,
    isLimited: limitedItemData?.IsLimited,
    isLimitedUnique: limitedItemData?.IsLimitedUnique,
    imageUrl: thumbUrl
  };

  if (editLimitedId !== null) {
    const updated = [...limiteds];
    updated[editLimitedId] = newItem;
    setLimiteds(updated);
    setEditLimitedId(null);
  } else {
    setLimiteds([...limiteds, newItem]);
  }

  setLimitedForm({ assetId: '', price: '', contactLink: '', sellerName: seller?.username || '' });
  setLimitedItemData(null);
  setThumbUrl('');
};

const handleLimitedEdit = (index) => {
  const item = limiteds[index];
  setLimitedForm({
  ...
  setEditLimitedId(index);
  fetchLimitedInfo();
};

const handleLimitedDelete = (index) => {
  const updated = limiteds.filter((_, i) => i !== index);
  setLimiteds(updated);
};

const filteredLimiteds = limiteds.filter(item =>
  item.itemName?.toLowerCase().includes(limitedSearch.toLowerCase())
);



{/* Limiteds Listing Form - Aligned Next to Robux Form */}
<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
  {/* Existing Robux Listing Form should be on the left - we assume it's already there */}

  {/* Limiteds Form */}
  <div className="p-4 border rounded shadow">
    <h2 className="text-xl font-semibold mb-4">Add Limited Listing</h2>

    <input
      type="text"
      placeholder="Asset ID"
      value={limitedForm.assetId}
      onChange={(e) => setLimitedForm({ ...limitedForm, assetId: e.target.value })}
      className="w-full border px-3 py-2 mb-2"
    />
    <button
      className="bg-blue-600 text-white px-4 py-2 rounded mb-4"
      onClick={fetchLimitedInfo}
    >
      Auto-Fetch Info
    </button>
    ...
    {limitedItemData && (
      <div className="mb-4">
        <img src={thumbUrl} alt="Thumbnail" className="w-24 h-24 object-cover mb-2 rounded" />
        <p><strong>Name:</strong> {limitedItemData.Name}</p>
        <p><strong>Creator:</strong> {limitedItemData.Creator?.Name}</p>
        <p><strong>Is Limited:</strong> {limitedItemData.IsLimited ? 'Yes' : 'No'}</p>
        <p><strong>Is Limited Unique:</strong> {limitedItemData.IsLimitedUnique ? 'Yes' : 'No'}</p>
      </div>
    )}
    ...
    <input
      type="text"
      placeholder="Price"
      value={limitedForm.price}
      onChange={(e) => setLimitedForm({ ...limitedForm, price: e.target.value })}
      className="w-full border px-3 py-2 mb-2"
    />
    <input
      type="text"
      placeholder="Contact Link"
      value={limitedForm.contactLink}
      onChange={(e) => setLimitedForm({ ...limitedForm, contactLink: e.target.value })}
      className="w-full border px-3 py-2 mb-2"
    />
    <input
      type="text"
      value={limitedForm.sellerName}
      disabled
      className="w-full border px-3 py-2 mb-4 bg-gray-100"
    />

    <button
      onClick={handleLimitedSubmit}
      className="bg-green-600 text-white px-4 py-2 rounded w-full"
    >
      {editLimitedId !== null ? 'Update Listing' : 'Submit Listing'}
    </button>
  </div>
</div>

{/* Search + Limiteds Table */}
<div className="mt-10">
  <h3 className="text-lg font-bold mb-2">Listed Limited Items</h3>
  <input
    type="text"
  ...
  <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
    {filteredLimiteds.map((item, index) => (
      <div key={index} className="border rounded p-4 shadow">
        <img src={item.imageUrl} alt="thumb" className="w-24 h-24 rounded mb-2" />
        <p><strong>Name:</strong> {item.itemName}</p>
        <p><strong>Creator:</strong> {item.creator}</p>
        <p><strong>Limited:</strong> {item.isLimited ? 'Yes' : 'No'}</p>
        <p><strong>Unique:</strong> {item.isLimitedUnique ? 'Yes' : 'No'}</p>
        <p><strong>Price:</strong> {item.price}</p>
        <p><strong>Contact:</strong> <a href={item.contactLink} className="text-blue-600 underline">Link</a></p>
        <p><strong>Seller:</strong> {item.sellerName}</p>
        <div className="flex space-x-2 mt-2">
          <button
            className="bg-yellow-500 text-white px-3 py-1 rounded"
            onClick={() => handleLimitedEdit(index)}
          >
            Edit
          </button>
          <button
            className="bg-red-600 text-white px-3 py-1 rounded"
            onClick={() => handleLimitedDelete(index)}
          >
            Delete
          </button>
        </div>
