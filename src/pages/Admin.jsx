  ...
  const addAccount = async () => {
    ...
    if (res.ok) {
      ...
      setForm({
        ...
        mop: 'Gcash', negotiable: 'Yes', robuxBalance: '', limitedItems: '', inventory: 'Public', games: ['', '', '']
      });
    } else {
    ...
    <div className="container" style={{ maxWidth: '500px', margin: '0 auto', padding: '20px' }}>
      {!isLoggedIn ? (
        ...
        <>
          ...
          {/* Games Input */}
          {[0,1,2].map(i => (
            <input key={i} type="text" placeholder={`Game ${i+1}`} value={form.games[i]} 
              onChange={e => {
                const updatedGames = [...form.games];
                updatedGames[i] = e.target.value;
                setForm({...form, games: updatedGames});
              }} 
              style={{ width: '100%', padding: '10px', marginBottom: '10px' }}
            />
          ))}

          <button onClick={addAccount} style={{ padding: '10px 20px', background: 'green', color: '#fff', border: 'none' }}>
            Add Account
          </button>
          ...
          {accounts.map(acc => (
            <div key={acc.id} style={{ border: '1px solid #ccc', padding: '10px', marginTop: '10px' }}>
              ...
              <button onClick={() => deleteAccount(acc.id)} style={{ padding: '5px 10px', background: 'red', color: '#fff', border: 'none' }}>
                Delete
              </button>
            </div>
          ))}
