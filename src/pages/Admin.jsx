...
export default function Admin() {
  ...
  const handleLimitedChange = (e) => {
    setLimitedForm({ ...limitedForm, [e.target.name]: e.target.value });
  };

  const fetchLimitedDetails = async () => {
    try {
      const res = await fetch(\`https://economy.roblox.com/v2/assets/\${limitedForm.assetId}/details\`);
      const data = await res.json();
      const thumb = await fetch(\`https://thumbnails.roblox.com/v1/assets?assetIds=\${limitedForm.assetId}&size=420x420&format=Png\`);
      const thumbData = await thumb.json();

      setLimitedForm({
        ...limitedForm,
        itemName: data[0]?.Name || '',
        creator: data[0]?.Creator?.Name || '',
        isLimited: data[0]?.IsLimited || false,
        isLimitedUnique: data[0]?.IsLimitedUnique || false,
        imageUrl: thumbData.data[0]?.imageUrl || '',
        sellerName: seller?.username || ''
      });

      Swal.fire('Fetched!', 'Limited item data has been auto-filled.', 'success');
    } catch (error) {
  ...
  const handleLimitedSubmit = async () => {
    try {
      const response = await fetch('/api/limiteds', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      Swal.fire('Success', 'Limited listing submitted!', 'success');
      setLimitedForm({
        assetId: '',
        itemName: '',
        creator: '',
