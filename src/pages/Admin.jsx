  ...
  if (!isAuthorized && !seller) {
    return (<>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
          <button onClick={() => setFormType('account')} className={formType === 'account' ? 'active-tab' : ''}>Account Listing Form</button>
          <button onClick={() => setFormType('robux')} className={formType === 'robux' ? 'active-tab' : ''}>Robux Listing Form</button>
          <button onClick={() => setFormType('limited')} className={formType === 'limited' ? 'active-tab' : ''}>Limited Listing Form</button>
        </div>
    ...
    <div className="container" style={{ padding: '20px' }}>
      ...
      {/* === ROBUX FORM SECTION === */}
        ...
        {formType === 'limited' && (
          <div>
            <h2 style={{ color: 'white' }}>{limitedEditMode ? 'Edit' : 'Submit'} Limited Listing</h2>
            <input placeholder="Item Name" value={limitedFormData.itemName} onChange={(e) => setLimitedFormData({ ...limitedFormData, itemName: e.target.value })} />
            <input placeholder="RAP" value={limitedFormData.rap} onChange={(e) => setLimitedFormData({ ...limitedFormData, rap: e.target.value })} />
            <input placeholder="Price" value={limitedFormData.price} onChange={(e) => setLimitedFormData({ ...limitedFormData, price: e.target.value })} />
            <input placeholder="Contact" value={limitedFormData.contact} onChange={(e) => setLimitedFormData({ ...limitedFormData, contact: e.target.value })} />
            <button>{limitedEditMode ? 'Update' : 'Submit'}</button>

            <h3 style={{ color: 'white' }}>Limited Listings</h3>
            {limitedListings.map((item) => (
              <div key={item._id} className="listing-card">
                <p><strong>{item.itemName}</strong> - RAP: {item.rap}, Price: {item.price}</p>
