...
export default function Admin() {
  ...
  const handleLimitedSubmit = async (e) => {
    ...
    try {
      const res = await fetch("/api/limited", {
        ...
        body: JSON.stringify({ ...limitedForm, seller: seller?.username })
      });
      const data = await res.json();
      if (data.success) {
        Swal.fire("Success", "Limited item listed!", "success");
        setLimitedForm({ assetId: "", price: "", mop: "Gcash" });
      } else {
  ...
  if (!isAuthorized && !seller) {
    ...
    <div className="container" style={{ padding: '20px' }}>
      ...
      {formType === 'limited' && (
        <form onSubmit={handleLimitedSubmit} className="form">
          <h3 style={{ color: 'white' }}>Limited Item Listing</h3>
          <input type="text" placeholder="Asset ID" value={limitedForm.assetId} onChange={(e) => setLimitedForm({ ...limitedForm, assetId: e.target.value })} required />
          <input type="text" placeholder="Price" value={limitedForm.price} onChange={(e) => setLimitedForm({ ...limitedForm, price: e.target.value })} required />
          <input type="text" placeholder="MOP (Gcash/Paymaya/etc)" value={limitedForm.mop} onChange={(e) => setLimitedForm({ ...limitedForm, mop: e.target.value })} required />
          <button type="submit" disabled={isSubmitting}>{isSubmitting ? "Submitting..." : "Submit Limited Item"}</button>
        </form>
      )}
